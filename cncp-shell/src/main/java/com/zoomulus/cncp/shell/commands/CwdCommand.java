...
public class CwdCommand implements Command {
    ...
    @Inject
    public CwdCommand(@NotNull final FileSystem fs) {
        this.fs = fs;
    }

    @Override
    public @NotNull List<String> getNames() {
        return Lists.newArrayList("cwd", "pwd");
    }

    @Override
    public @NotNull String getShortDescription() {
        return "Shows the full path of the current working directory";
    }
